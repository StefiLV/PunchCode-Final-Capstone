...
public class JdbcMessageDao implements MessageDao {
    ...
    public void createMsg(Message newMsg){
        ...
        jdbcMsgTemplate.update(sql, newMsg.getTimeStamp(), newMsg.getText(), newMsg.getAttachment(), newMsg.isArchive());
    }
    @Override
    public boolean updateMsg(Message msg, int id){
        String sql = "UPDATE message SET time_stamp = ?, message_text = ?, attachment = ?, archive = ? ";
        return jdbcMsgTemplate.update(msg.getText(), msg.getTimeStamp(), msg.getAttachment(), msg.isArchive()) == 1;
    }
    @Override
    ...
    private Message mapRowToMessage(SqlRowSet mg){
        ...
        io.setText(mg.getString("message_text"));
